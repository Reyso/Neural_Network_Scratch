...
class Neural:
    ...
    def initialize_layers(self) -> List[Tuple[np.ndarray, np.ndarray]]:
        ...
        for i in range(1, len(self._layer_structure)):
            ...
            # Correção na inicialização dos biases para corresponder ao número de neurônios na camada atual
            biases = np.ones((1, self._layer_structure[i]))  # Esta linha precisa ser ajustada
