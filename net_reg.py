...
class Neural:
    ...
    def fit(self, X: np.ndarray, y: np.ndarray, lambda_: float = 0.0) -> None:
        # Validation split
        ...
        for epoch in range(self._epochs):
            ...
            for i in range(1, len(self._layers)):
                ...
                # Calculate loss
                loss = self.calculate_loss(y_batch, pred, lambda_)
                epoch_losses.append(np.mean(loss ** 2))

                # Backward
                self.backward(hidden, loss, lambda_)
    ...
    def backward(self, hidden: List[np.ndarray], grad: np.ndarray, lambda_: float) -> None:
    ...
    def calculate_loss(self, actual: np.ndarray, predicted: np.ndarray, lambda_: float) -> np.ndarray:
        "mse with L2 regularization"
        loss = predicted - actual
        regularization_term = 0.0
        for i in range(len(self._layers)):
            weights, _ = self._layers[i]
            regularization_term += np.sum(weights ** 2)
        loss += (lambda_ / 2) * regularization_term
        return loss
